...
# Debug controls
DEBUG_MAX_SUBJECTS = 5
DEBUG_EEG_ONLY = True          # True -> stop after EEG epoch loading summary
DEBUG_EEG_FEATURES_ONLY = False  # True -> stop after EEG feature matrix build
DEBUG_PRINT_CHANNELS = True    # print selected channels for first loaded subject
...
def pick_first_n_channels(raw: mne.io.BaseRaw, n_channels: int = EEG_CHANNELS) -> List[str]:
    ...
    You can later replace this with explicit channel-name selection.
    """
...
def load_eeg_from_ds005555(
    ...
    for sub_dir in subject_dirs:
        ...
        if is_probably_placeholder(edf_path):
            print(f"[EEG] Skipping {sub_dir.name}: EDF looks like placeholder/unusable file ({edf_path.stat().st_size} bytes)")
            subjects_skipped += 1
            continue
