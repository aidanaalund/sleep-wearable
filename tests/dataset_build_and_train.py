...
"""
Dataset-building and training entrypoint for the deployable two-model setup.

Assumptions:
- EEG and IMU come from TWO DIFFERENT datasets.
- IMU dataset is stored under DataPaper/user_1 ... DataPaper/user_22.
- Each IMU user folder contains:
    - Actigraph.csv
    - Sleep.csv
- Training/export is done offline.
- Runtime fusion happens outside ONNX.

What this script does:
1) Build EEG training arrays: X_eeg, y_stage
2) Build IMU training arrays: X_imu, y_sleep
3) Train sklearn pipelines
4) Export ONNX models
5) Save feature schemas and label maps
"""
...
def is_probably_placeholder(path: Path, min_bytes: int = 4096) -> bool:
    """
    Heuristic for DataLad/git-annex placeholder or unfetched file.
    """
    try:
        if not path.exists():
            return True
...
def build_eeg_dataset(eeg_epochs: Sequence[np.ndarray], eeg_labels: Sequence[str]) -> Tuple[np.ndarray, np.ndarray]:
    """
    Generic builder for an EEG dataset that is already epochized.

    Expected inputs:
    - eeg_epochs: iterable of arrays shaped (EEG_CHANNELS, n_samples)
    - eeg_labels: iterable of string labels in STAGE_LABELS
    """
    X: List[np.ndarray] = []
    y: List[str] = []
    ...
    if not X:
        raise ValueError("No EEG samples were built. Check your EEG loader.")
...
def load_eeg_from_ds005555(
    ...
    """
    Loads 30-second EEG epochs and stage labels from OpenNeuro ds005555.

    Uses:
      - headband EDF for EEG signal
      - PSG events TSV for labels (stage_hum)
    """
    ...
    for sub_dir in subject_dirs:
        ...
        event_files = sorted(eeg_dir.glob("*_task-Sleep_acq-psg_events.tsv"))

        if not edf_files or not event_files:
        ...
        if is_probably_placeholder(edf_path):
            print(f"[EEG] Skipping {sub_dir.name}: EDF looks like placeholder/unfetched file")
            subjects_skipped += 1
            continue
        ...
        picks = raw.ch_names[:EEG_CHANNELS]
        if len(picks) < EEG_CHANNELS:
            print(f"[EEG] Skipping {sub_dir.name}: only {len(picks)} channels found, need {EEG_CHANNELS}")
            subjects_skipped += 1
            continue
        ...
        if "stage_hum" not in events_df.columns:
            print(f"[EEG] Skipping {sub_dir.name}: stage_hum not found in {events_path.name}")
            subjects_skipped += 1
            continue

        if "onset" not in events_df.columns:
            print(f"[EEG] Skipping {sub_dir.name}: onset not found in {events_path.name}")
            subjects_skipped += 1
            continue
    ...
    if not eeg_epochs:
        raise ValueError("No EEG epochs were built from ds005555. Check fetch state, event labels, and channels.")
...
def build_imu_dataset(data_paper_root: Path) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Builds IMU training arrays from the 22-user DataPaper directory.
    """
    X: List[np.ndarray] = []
    y: List[int] = []
    groups: List[str] = []
    ...
    for user_dir in user_dirs:
        ...
        for start in range(0, len(accel) - epoch_len + 1, epoch_len):
            ...
            # Majority vote is usually more stable than "any-sample sleep"
...
def main() -> None:
    ...
    ds005555_root = Path("ds005555/ds005555-main")

    eeg_epochs, eeg_labels = load_eeg_from_ds005555(
        ds_root=ds005555_root,
        max_subjects=5,   # set to None when scaling up
        strict=False,
    )
